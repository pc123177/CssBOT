...
class MultiUserBot:
    def __init__(self, store, invite_code: str, admin_chat_id: str, sender):
        self.store = store
        self.invite_code = invite_code
        self.admin_chat_id = str(admin_chat_id)
        self.sender = sender

    def command(self, chat_id: str, name: str, text: str) -> str:
        chat_id = str(chat_id)
        ...
        if command == "/start":
            ...
            self.store.register(chat_id, name)
            return f"✅ {name or 'Usuário'} cadastrado. {HELP}"

        if command in ("/usuarios", "/bloquear", "/broadcast"):
            ...
            if command == "/bloquear":
                self.store.unsubscribe(argument.strip())
